...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return balanced(left_end) and balanced(right_end) and length(left_arm) * total_weight(left_end) == length(right_arm) * total_weight(right_end)
...
def sub_interval(x, y):
    ...
    p3 = upper_bound(x) - lower_bound(y)
    p4 = upper_bound(x) - upper_bound(y)
    min_sub = min(p1, p2, p3, p4)
    max_sub = max(p1, p2, p3, p4)
    return interval(min_sub, max_sub)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) * upper_bound(y) > 0, 'Error: Division by an interval that spans zero'
