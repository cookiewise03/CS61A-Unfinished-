    ...
    def vend(self):
        if self.stock == 0:
            ...
            if self.balance < self.price:
                ...
                return  'You must add ${0} more funds.'.format(needed)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        year_difference = Mint.current_year -self.year
        value = self.cents + max(year_difference - 50, 0)
        return value
...
def store_digits(n):
    ...
    remain_digits = n % divisor
    return Link(first_digit, store_digits(remain_digits))
