...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        orig_player_deck_length = len(player.deck.cards)
        for i in range(orig_player_deck_length):
            player.deck.cards[i].attack += other_card.attack
            player.deck.cards[i].defense += other_card.defense
        opponent_deck_length = orig_opponent_deck_length
        while opponent_deck_length:
            for j in range(opponent_deck_length):
                attack_in_deck = opponent.deck.cards[j].attack
                defense_in_deck = opponent.deck.cards[j].defense
                if attack_in_deck == other_card.attack or defense_in_deck == other_card.defense:
                    opponent.deck.cards.pop(j)
                    break
            opponent_deck_length -= 1
