...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def include_topic_words(paragragh):
        paragragh_words = split(lower(remove_punctuation(paragragh)))
        for word in topic:
            if word in paragragh_words:
                return True
        return False
    return include_topic_words
    # END PROBLEM 2
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    elif all(diff_function(user_word, valid_word, limit) > limit for valid_word in valid_words):
        return user_word
    else:
