...
def swine_align(player_score, opponent_score):
    ...
    if (player_score >0 and opponent_score >0):
        cd_check = False
        cd = min(player_score, opponent_score)
        while cd > 0:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_rolls in range(1, 11):
        ...
        current_average = averaged_roll_dice(num_rolls, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    potential_score = score + bacon_points
    if extra_turn(potential_score, opponent_score):
        return 0
    elif bacon_points >= cutoff:
        return 0
    else:
